...
pub fn vincenty_inverse(sp_lat_rad: f64, sp_lon_rad: f64, ep_lat_rad: f64, ep_lon_rad: f64, el: &Ellipsoid, eps: f64, it_limit: i32) -> (f64, f64, f64, i32, bool) {
    ...
    let dst_m = el.mnsa_m * a_ * (   - delta_sigma);
...
fn point_to_geodesic(mut p_a: (f64, f64), p_b: (f64, f64), p_p: (f64, f64)) -> (f64, f64) {
    let earth = Ellipsoid::from_descriptor(&WGS84_ELLIPSOID_DESCRIPTOR);  
    let mut p_a2: (f64, f64, f64, i32);
    loop {
        ...
        */
        p_a2 = vincenty_direct(p_a.0, p_a.1, ab.1, ap.0,  &earth, 0.5, 32767); //placeholder
...
fn test(p_a: (f64, f64), p_b: (f64, f64), p_p: (f64, f64)) {
